...
1 堆针对一个JVM进程来说是唯一的，也就是**一个进程只有一个JVM**，一个jvm就是一个进程但是进程包含多个线程，他们是共享同一堆空间的。
...
6 所有的线程共享Java堆，在这里还可以划分**线程私有的缓冲区**（Thread Local Allocation Buffer，TLAB）。
...
我要说的是：**“几乎”**所有的对象实例都在这里分配内存。**因为还有一些对象是在栈上分配的**

数组和对象**可能**永远不会存储在栈上，因为栈帧中保存引用，这个引用指向对象或者数组在堆中的位置。
...
Java堆区用于存储Java对象实例，那么**堆的大小在JVM启动时就已经设定好了**，大家可以通过选项"-Xmx"和"-Xms"来进行设置。
...
通常会将-Xms和-Xmx两个参数配置相同的值，其目的是**为了能够在java垃圾回收机制清理完堆区后不需要重新分隔计算堆区的大小，从而提高性能**（一开始就把最大的给你，别老是找我要了）。
...
可以设置参数：-Xx:MaxTenuringThreshold= N  进行设置N次之后去老年代
...
- Minor GC：新生代的GC
- Major GC：老年代的GC
- Full GC：整堆收集，收集整个Java堆和方法区的垃圾收集（**避免使用**）原因如下：
...
- 调用System.gc（）时，系统**建议**执行Fu11GC，但是不必然执行
- 老年代空间不足
- 方法区空间不足
- 通过Minor GC后进入老年代的平均大小大于老年代的可用内存
- 由Eden区和（From Space）区向（To Space）区复制时，对象大小大于To Space可用内存，则把该对象转存到老年代，且老年代的可用内存小于该对象大小

开发指导**：Full GC 是开发或调优中尽量要避免的**。这样暂停时间会短一些
...
尽管不是所有的对象实例都能够在TLAB中成功分配内存（因为TLAB空间太小了），==**但JVM确实是将TLAB作为内存分配的首选。**==
...
1在发生Minor GC之前，虚拟机会检查老年代最大可用的连续空间是否大于新生代所有对象的总空间。

- 1.1如果大于，则此次Minor GC是安全的
- 1.2如果小于，则虚拟机会查看-xx:HandlePromotionFailure设置值是否允担保失败。
  - 1.2.1如果HandlePromotionFailure=true，会继续检查老年代最大可用连续空间是否大于历次晋升到老年代的对象的平均大小。
  - ​        1.2.1.1如果大于，则尝试进行一次Minor GC，但这次Minor GC依然是有风险的；
  - ​        1.2.1.2如果小于，则改为进行一次FullGC。
  - 1.2.2如果HandlePromotionFailure=false，则改为进行一次FullGc。
...
==**JDK7之后：只要老年代的连续空间大于新生代对象总大小或者历次晋升的平均大小就会进行Minor GC，否则将进行FullGC。**==
...
在Java虚拟机中，对象是在Java堆中分配内存的，这是一个普遍的常识。但是，有一种特殊情况，那就是如果经过**逃逸分析**（Escape Analysis）后发现，**一个对象并没有逃逸出方法的话，那**么就可能被优化成栈上分配。这样就无需在堆上分配内存，也无须进行垃圾回收了。这也是最常见==的**堆外存储技术**==。

此外，前面提到的基于openJDk深度定制的TaoBaovm，其中创新的**GCIH（GC invisible heap）**技术实现off-heap，将生命周期较长的Java对象从heap中移至heap外，并且GC不能管理GCIH内部的Java对象，以此达到降低GC的回收频率和提升GC的回收效率的目的。
...
public static String createStringBuffer(String s1, String s2) {
 ...
 sb.append(s1);
 sb.append(s2);
 return sb.toString();//sb.toString()实际得到一个新的和sb内容相同的string对象，然后将其返回。sb仍然在栈中没有逃逸。
}
```
...
public class EscapeAnalysis {
    ...
    public void useEscapeAnalysis2() {
        EscapeAnalysis e = getInstance();//因为getInstance()得到外部的成员变量，从外面传进来的。肯定会逃逸
        // getInstance().XXX  同样会发生逃逸
    }
}
```

==总结：如何快速的判断是否发生了逃逸分析，就看new的对象实体是否在方法外被调用。==
...
在动态编译同步块的时候，JIT编译器可以借助逃逸分析来判断同步块所使用的锁对象是否只能够被一个线程访问而没有被发布到其他线程**。如果是只能被一个线程访问，**那么JIT编译器在编译这个同步块的时候就会取消对这部分代码的同步。这样就能大大提高并发性和性能。这个取消同步的过程就叫同步省略，也叫锁消除。
...
老年代放置长生命周期的对象，通常都是从survivor区域筛选拷贝过来的Java对象。当然，也有特殊情况，
